...
# path = "./result/greedy/" + datetime.datetime.now().strftime("%m-%d_%H-%M-%S_") + description_str + "/"
path = "./result/greedy/" + description_str + "/"
...
MAX_EPISODES = 510 #不用訓練到80000，可以訓練3000
MAX_EP_STEPS = 400
    ...
    for j in range(MAX_EP_STEPS):
        ...
        # migration
            ...
            for k in range(env2.get_SFC_len()):
                ...
                # a = np.clip(np.random.normal(a, var), 1e-2, 1 - 1e-2)
                # a = np.random.random((1, env2.a_dim))
                s_, r,already_satisfy = env2.greedy_step()
                # if env2.no_resource == True:
                #     s = s_
        ...
        if j == MAX_EP_STEPS - 1:
            ...
            print("----------------------------------------------------")
            print(f"Episode = {i} 花費時間:{round((time.time()-ep_start_time), 3)}秒")
            print("----------------------------------------------------")
            ...
            print(f"m_mig_energy:{m_mig_energy/m_event_num}")
            ...
            print(f'沒有搬移:{env2.no_change}/{m_event_num*2}={env2.no_change/m_event_num*2}')
            print(f"一個episode有{env2.bigger_E2E_delay}個event違反TD conds") #，因為不搬移得到更大的TD，平均多{env2.diff_E2E_delay/env2.bigger_E2E_delay}ms")
            print(f"一個episode有{env2.data_rate_violate}個event違反data_rate需求")
            print(f"一個episode有{env2.affects_other_SFC}個event造成其他SFC違反TD限制")
